 ...
 return <>
  ...
  {adding&&<Modal title="Nuevo inquilino" onClose={()=>setAdding(false)}>
   <form onSubmit={handleSubmit(save)} className="grid gap-4 sm:grid-cols-2">
    ...
    <button disabled={isSubmitting} className="btn-primary mt-1 w-full sm:col-span-2">{isSubmitting?'Guardando...':'Guardar inquilino'}</button>
   </form>
  </Modal>}
 </>
}
 ...
 return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
  {tenants.map(tenant=>{
   ...
   return <button key={tenant.id} onClick={()=>onSelect(tenant)} className="card min-w-0 p-4 text-left transition hover:-translate-y-0.5 hover:border-brand-300 sm:p-5">
    ...
    <p className="break-words text-sm capitalize text-stone-500">{tenant.tipo||'persona'} - {property?.codigo||'Sin espacio'}</p>
...
function TenantDetail({tenant,property,movements,isAdmin,onDelete,onClose}){
 return <Modal title="Detalle del inquilino" onClose={onClose}>
  ...
  {isAdmin&&<div className="mt-4 flex justify-end">
   <button type="button" onClick={onDelete} className="btn-secondary w-full text-red-700 hover:bg-red-50 hover:text-red-800 sm:w-auto"><Trash2 size={18}/> Eliminar inquilino</button>
  </div>}

  <h3 className="mb-3 mt-6 flex items-center gap-2 font-bold"><Receipt size={18}/> Movimientos asociados</h3>
  {!movements.length?<Empty title="Sin movimientos registrados" text="Los movimientos asociados a este inquilino apareceran aqui."/>:<div className="space-y-2">{movements.map(x=><div key={x.id} className="flex min-w-0 flex-col gap-1 rounded-xl bg-white p-3 min-[380px]:flex-row min-[380px]:justify-between min-[380px]:gap-3"><span className="min-w-0 break-words text-sm sm:text-base">{datePE(x.fechaMovimiento)} - {x.concepto}</span><b className={`shrink-0 whitespace-nowrap ${x.tipo==='ingreso'?'text-income':'text-expense'}`}>{x.tipo==='ingreso'?'+':'-'} {money(x.monto)}</b></div>)}</div>}
 </Modal>
}
