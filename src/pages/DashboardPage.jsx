 ...
 return <>
  <PageHeader
   ...
   action={<div className="flex gap-2">
    <select className="field !py-2" value={month} onChange={event=>setMonth(event.target.value)}>
    ...
    <select className="field !py-2" value={year} onChange={event=>setYear(event.target.value)}>
     {[now.getFullYear(),now.getFullYear()-1,now.getFullYear()-2].map(item=><option key={item}>{item}</option>)}
    </select>
   </div>}
  />

  {isAdmin&&<div className="mb-6 grid grid-cols-2 gap-3 sm:hidden">
   <Link to="/movimientos/nuevo?tipo=ingreso" className="btn-primary"><Plus size={18}/> Ingreso</Link>
   <Link to="/movimientos/nuevo?tipo=gasto" className="btn-secondary"><Plus size={18}/> Gasto</Link>
  </div>}
  ...
  <div className="mt-6 grid gap-6 xl:grid-cols-2">
   ...
   <section className="card p-5">
    ...
    <div className="mt-5 h-64">
     <ResponsiveContainer><BarChart data={six}>
  ...
  <div className="mt-6">
   <section className="card overflow-hidden">
    ...
    {!filtered.length?<div className="p-5"><Empty title="Sin movimientos este mes" text="Los ingresos y gastos apareceran aqui."/></div>:<div className="divide-y divide-stone-100">
     {filtered.slice(-5).reverse().map(item=><div key={item.id} className="flex items-center justify-between gap-3 px-5 py-4">
      <div><p className="font-semibold">{item.concepto}</p><p className="text-sm text-stone-500">{datePE(item.fechaMovimiento)} · {item.metodoPago}</p></div>
      <b className={item.tipo==='ingreso'?'text-income':'text-expense'}>{item.tipo==='ingreso'?'+':'-'} {money(item.monto)}</b>
     </div>)}
    </div>}
   </section>
  </div>
